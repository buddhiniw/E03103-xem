...
! DESCRIPTION:
!
! This routine calculates the convolution of F2_N(x/z,qsq) with f_A(z,beta)
! following the procedure of Benhar, et.al. (from Jun 1997 preprint).
!
! F2_A(x,qsq)=Integral[from x to A] of f_A(z,beta),F2_N(x/z,qsq)
!
! F2_N is the nucleon structure function for a nucleon with momentum z
! and a quark momentum fraction x/z.  Beta is defined as abs(q3)/nu.
! Here, we choose beta as a function of scattering angle only, such
! that the fit matches the data, and expected ratio of DIS/QE from other
! calculations.
!
! The f(z,beta) is taken from tables (generated by Benhar), for beta values
! of 1.0,1.1,...,1.5.  They are calculated for Nuclear Matter (?).
!
! F2_N(x/z,qsq) from bodel fits to proton and neutron.  Use N=Z=A/2 to
! get symmetric nucleus with A nucleons.  Correct for neutron excess and
! EMC effect with fit to EMC data.
!
! If smearflag=.false., return Z*f2_p + (A-Z)*f2_n - no smearing, no EMC.
! (Probably should have EMC, but I don't plan on using unsmeared).
!
!
!
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c (intpts)-point integration of f(z)*F2(x/z).  f(z) normalized to one, so no
c  explicit check of normalization.  Truncate integral at z=5 (since f(z)=1e-10)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	  amax=min(5.,anuc)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c calculate modified kinematics to get F2(x/z,qsq).  Fix x_q=x/z, qsq, and theta
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	   ...
	   if (abs(qsq-qsq_test)/qsq.gt.1e-4.or.abs(xq-x_test)/xq.gt.1e-4) then
	    ...
	    if (anuc.lt.3.and.anuc.gt.1.) then 
	       fz=fz_table(1.2*(z-1)+1,beta) !20% narrower for deut.
c	    elseif(anuc.lt.5.0.and.anuc.gt.2.) then
c	       fz=fz_table(1.1*(z-1)+1,beta) !10% narrower for he3/he4.
	    endif
